<template>
  <div class="photo">
    ...
    <div class="header"  @click= "triggerClick">
      <div class="content" >
        {{hasComment || this.type === 'parent' ? '查看图片' : '添加图片'}}
      </div>
      <div class="head-img">
    ...
    <div class="show-img-ctn vux-1px-t" v-if="this.hasComment">
      <div class="imgs" >
        <div class="img-item img-zoom" v-for="(item, index) in this.viewImgList" :key="index" @click="show(index)">
          <img :src="item.src" alt="" class="previewer-demo-img">
        </div>
      </div>
      <div v-transfer-dom>
        <previewer :list="this.viewImgList" ref="previewer" :options="options" @on-index-change="logIndexChange">
    ...
    <div class="show-img-ctn vux-1px-t" v-else>
      ...
      <div v-transfer-dom>
        <previewer :list="fileList" ref="previewer" :options="options" @on-index-change="logIndexChange">
...
<div class="no-data" v-if="type === 'parent' && !viewImgList.length">
    暂无数据
</div>
</div>
