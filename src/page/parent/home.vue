        ...
        <div class="info">
          ...
          <div class="progressed">
            ...
            <box gap="10px" class="box">
              ...
              <div class="point" :style="'left:' + (percent > 2.5 ? (percent - 2.5) : 0) + '%'" v-if="complete!==0"></div>
            </box>
            <div class="total">
              共{{total}}节课
            </div>
      ...
      <div class="bottom-class">
        ...
        <div class="class-content">
          <div class="class-week">
            <div class="tab-item" v-for="(item, index) in tabArr" :key="index" @click="changeActive(index)" :class="index === activeIndex && 'active'">
              {{item}}
            </div>
          </div>
          <div class="course-ctn list-wrapper"  ref="wrapper" >
            <div>
              <div class="courses"  v-for="(item,index) in weekClassData[(activeIndex + 1) % 7]" :key="index" @click="routerChange(index)">
              ...
              <div v-if="!(weekClassData[(activeIndex + 1) % 7] &&weekClassData[(activeIndex + 1) % 7].length )" class="noClass">暂无课程安排</div>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    initScroll() {
      ...
      const topHeight = util.getStyle(this.$el.querySelector('.parent-top'), 'height')
      const historyHeight = util.getStyle(this.$el.querySelector('.bottom-history'), 'height')
      const titleHeight = util.getStyle(this.$el.querySelector('.class-title'), 'height')
      const week = util.getStyle(this.$el.querySelector('.class-week'), 'height')
      const wrapperHeight = winHeight - parseFloat(topHeight) - parseFloat(historyHeight) - parseFloat(titleHeight) - parseFloat(week)
    ...
    routerChange(index) {
      // 保存当前查看课程
      this.$store.commit('saveCurrentCourse',this.weekClassData[(this.activeIndex + 1) % 7][index])
    ...
    getWeekData() {
      ...
      if (time.getDay !== 0) {
        let startDate = new Date(pureDate.getTime() - (time.getDay() - 1) * ONE_DAY)
        let endDate = new Date(pureDate.getTime() + (7 - time.getDay()) * ONE_DAY)
    ...
    // 获取学生信息
    getData() {
      API.homeAPI.wxAppGetStudent({sid:this.userInfo.sid})
      .then((data) => {
          if (data) {
            this.infoData = data
            this.complete = this.infoData.period_total - this.infoData.period_surplus
